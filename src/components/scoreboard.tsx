...
export function sortAndRankTeams(teams: ScoreboardRow[]): ScoreboardRow[] {
  ...
  teams.sort((a, b) => {
    ...
    const aTime = Math.max(
      ...a?.problems?.filter((pr) => pr?.time)?.map((t) => t?.time),
    );
    const bTime = Math.max(
      ...b?.problems?.filter((pr) => pr?.time)?.map((t) => t?.time),
    );
    return aTime - bTime;
  });
  ...
  teams.map((team) => {
    ...
    const maxSubTime = Math.max(
      ...team?.problems?.filter((pr) => pr?.time)?.map((t) => t?.time),
    );
    if (
      team.score.num_solved === lastSolved &&
      team.score.total_time === lastTime &&
...
export default function Scoreboard({ contestId }: ScoreboardProps) {
  ...
  const update = (
    ...
    if (!coolDown && Number(num_pending) > 0) {
      document
        .getElementById(`team-${teamId}-${prId}`)
    ...
    const problemAfter = updatedTeam.problems.find(
      (p) => p.problem_id === prId,
    );
  ...
  const getCellColor = (pr: ProblemInfo, team_id: string) => {
    ...
    if (pr.num_judged === 0 && prIsPend.num_pending > 0) {
      const totalSubs = subs?.filter(
        (sub) => sub.team_id == team_id && sub.problem_id == pr.problem_id,
      );
  ...
  const getNameTeam = (team_id: number | string) => {
    const team = teams?.find(
      (tm: { id: number; table: number }) =>
        tm.id == team_id || tm.lable == team_id,
    );
  ...
  const getUniTeam = (team_id: number | string) => {
    const team = teams?.find(
      (tm: any) => tm.id == team_id || tm.lable == team_id,
    );
    if (team?.hidden) return;
        ...
        <div className="min-w-[1024px]">
          <div className="grid grid-cols-[50px_25rem_10rem_1fr_5rem] min-h-10 place-items-stretch gap-2 px-2 text-lg font-bold bg-card-foreground/10 text-primary">
            ...
            <div className="text-center m-auto ">
              <span className="text-teal-900">SOLVED</span> /{" "}
              <span className="text-rose-900">TIME</span>
            </div>
            {
                    ...
                    {team.problems.map((pr) => (
                      ...
                      >
                        ...
                        <div className="text-[1rem] flex items-center font-normal justify-center gap-1">
                          ...
                          <span>
                            {pr.num_pending > 0 ? `+ ${pr.num_pending}` : ""}
                          </span>
                        </div>
                      </div>
                    ))}
